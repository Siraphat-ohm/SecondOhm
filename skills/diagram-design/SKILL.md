---
name: diagram-design
description: Create or redraw branded editorial diagrams as standalone HTML/SVG/PNG. Use for architecture, flow, process, data-model, timeline, chart, dependency, deployment, organization, or other explanatory diagrams; import draw.io or Mermaid; onboard brand tokens; optionally add annotations, motion, or sketch styling.
license: MIT
metadata:
  version: "2.6"
---
...
Before rendering, state the plan in one short message: the chosen visual type (and semantic pattern, if routed), the size preset, and anything the complexity budget (§7) will force out. Continue immediately when the request supplies enough content to make those choices; otherwise ask only for the missing decision. Record assumptions beside the deliverable.
...
**Geometry follows a 4px grid.** Coordinates, node dimensions, gaps, padding, and radii use the values below. Typography is role-based: use the type sizes set by the selected reference or style guide, including 7–9px technical labels.
...
Exempt: type-role font sizes, stroke widths (0.8, 1, 1.2), opacity values, and the 22×22 dot-pattern.

Quick check: geometry coordinates ending in 1, 2, 3, 5, 6, 7, or 9 need correction.
...
Run the **taste gate** after the HTML exists and before delivery. Every applicable item passes; `self_check.py` passing alone is insufficient.
...
- [ ] Every arrow label has an opaque mask resolving to the active `paper` role behind it?
- [ ] Legend is a horizontal bottom strip, not floating?
- [ ] No vertical `writing-mode` text?
- [ ] `viewBox` expanded for the legend strip (~60px)?
- [ ] Geometry coordinates, dimensions, gaps, padding, and radii follow the 4px grid; typography follows its role-specific size?
...
- [ ] Opened the generated HTML at its requested size in a browser and inspected the complete canvas: labels are legible, no paths or masks collide, and the legend is visible?
- [ ] If the output is animated, verified both the static/no-JavaScript meaning and the `prefers-reduced-motion: reduce` state?
...
1. Choose the visual type; if behavior is load-bearing, choose one semantic pattern first.
2. Load `references/style-guide.md`, the selected type reference, and `semantic-patterns.md` when routed. Choose the closest template (`assets/template.html` for minimal, `assets/template-full.html` for cards, `assets/template-motion.html` only when motion is requested).
...
5. Save the self-contained HTML, run `python3 scripts/self_check.py <file>`, then run the §9 taste gate including its rendered-result check.
